...
from LHM.models.rendering.gs_renderer import PointEmbed
    ...
    def infer_single_view(
        ...
        for view_idx in range(num_views):
            render_res = self.renderer.forward_animate_gs(
                ...
                # render_trans[:, view_idx : view_idx + 1],
                render_h,
                render_w,
...
class ModelHumanLRMSapdinoBodyHeadSD3_5(ModelHumanLRM):
    ...
    def __init__(self, **kwargs):
        ...
        self.render_h, self.render_w = 720, 1280
        self.lpips_loss_func = loss_utils.lpips_loss_fn()
    ...
    def forward_latent_points(self, image, head_image, camera, query_points=None):
        ...
        image_feats, head_feats, body_feats = self.forward_encode_image(
            image, head_image
        )  # image_feats (1,5120,1536), head_feats (1,3,448,448), body_feats (1,4096, 1536)
        ...
        tokens = self.forward_transformer(
            image_feats,
            camera_embeddings=None,    # mod
            query_points=query_points, # cond
    ...
    def infer_single_view(
        ...
        if self.latent_query_points_type.startswith("e2e_smplx"):
            query_points, smplx_params = self.renderer.get_query_points(
                smplx_params, device=image.device
            )  # 1,40000,3   预定义的那种，不是高斯的

        latent_points, image_feats = self.forward_latent_points(
            image[:, 0], head_image[:, 0], camera=None, query_points=query_points
        )  # [B, N, C]     # latent_points (1,40000,1024)       image_feats (1,5120,1536)
    ...
    def animation_infer(self, gs_model_list, query_points, smplx_params, render_c2ws, render_intrs, render_bg_colors,
                        model, gt_img, gt_msk, optimizer, scheduler, scaler):
        ...
        for view_idx in range(num_views):
            ...
            loss_dict['image_l1'], loss_dict['lpips'], loss_dict['mask_l2'], loss_dict['ssim'] = 0, 0, 0, 0
            metric_dict['train_psnr'], metric_dict['train_l1'] = 0, 0

            gt = gt_img[view_idx : view_idx + 1, :]#.squeeze()
            # render = render_res["comp_rgb"].squeeze().permute(1,2,0)
            render = render_res["comp_rgb"].squeeze(0).permute(0,2,3,1).squeeze(-1)
            render_msk = render_res["comp_mask"].squeeze(0).permute(0,2,3,1).squeeze(-1)
            msk = gt_msk[view_idx : view_idx + 1, :]

            libcore.write_tensor_image(os.path.join('./data', f'0-debug_render.jpg'), render.squeeze(), rgb2bgr=True)
            # exit(0)

            loss_dict['image_l1'] += (render.squeeze()[msk[0]==1] - gt.squeeze()[msk[0]==1]).abs().mean()
            loss_dict['mask_l2'] += ((render_msk - msk) ** 2).mean()
            loss_dict['lpips'] += (self.lpips_loss_func(render, gt).mean()) * 0.5
            # loss_dict['ssim'] += (ssim(render, gt).mean()) * 0.5
            metric_dict['train_psnr'] += ((psnr(render, gt).mean()))
            metric_dict['train_l1'] += (render.squeeze()[msk[0]==1] - gt.squeeze()[msk[0]==1]).abs().mean()
            total_loss = sum(loss_dict.values())

            optimizer_, scheduler_ = optimizer['train2D'], scheduler['train2D']
            # if enable_amp:
    ...
    def finetune_train(self, gs_model_list, query_points, smplx_params, render_c2ws, render_intrs, render_bg_colors,
                        model, gt_img, gt_msk, optimizer, scheduler, scaler):
        '''Inference code avoid repeat forward.
        '''
