...
class MetricComputer:
    def __init__(self, forward_bs=8):
        ...
        self.metrics = {
            'psnr': lambda x,y: psnr(x,y).squeeze(),
            'ssim': lambda x, y: ssim(x.permute(0,3,1,2),y.permute(0,3,1,2), window_size=11, size_average=False),
            'lpips': lambda x,y: lpips_fn(x.permute((0,3,1,2)),y.permute(0,3,1,2),normalize=True).squeeze()
        }
