...
def main(argv):
    ...
    while i < len(raw_tokens):
        ...
        if k in ('checkpoint-file', 'checkpoint_file') and cli_checkpoint_file is None:
            ...
            try:
                args.iter = int(value)
                args.iter_param = True  # Mark that iter was set from CLI
            except Exception:
                pass
    ...
    # For InterHand, we always use HandAvatarDataset (no input-dir needed)
    if not multi_image_mode:
        # test_hand = HandDataset(split='test_wild', img_path=img_glob[0]) if img_glob else None
    ...
    # ====================== Two-Stage Training: Inversion + Finetune ======================
    ...
    if inv_stage1_iters > 0:
        ...
        for step in inv_pbar:
            ...
            # Use run_wild_ohta for inversion stage (same as finetune_wild_ohta)
    ...
    if inv_stage2_iters > 0:
        ...
        # Optionally implement Stage 2 inversion with run_wild_inversion_stage2 if available
        ...
        if hasattr(runner, 'build_pseudo_gt_batch'):
            ...
            pseudo_batch = runner.build_pseudo_gt_batch(
                ...
                save_dir=save_dir,
                save_prefix=save_prefix,
                use_canonical_root=True,  # Use canonical pose for finetune
            )
    ...
    if finetune_iters > 0:
        if pseudo_batch is None:
            # Generate pseudo batch if not already created
            try:
                batches = next(data_iterator)
            except Exception:
                data_iterator = iter(test_dataloader)
        ...
        for step in finetune_pbar:
            ...
            # Use run_wild_stage_2 with pseudo_batch for pseudo-GT supervision
            runner.run_wild_stage_2_interhand(
                batch=pseudo_batch,  # Use pseudo-GT batch for supervision!
                scaler=scaler,
                iteration=step - 1,
                writer=writer,
                pbar=finetune_pbar,
            ...
            # Periodic testing and checkpointing
            total_step = inv_iters + step
            ...
            if do_test:
                ...
                # Infer GS from the test batch (take first element if it's a list from dataloader)
                ...
                # Test on HandAvatar dataset if available
    ...
    # If no finetune stage, still allow evaluation on inversion-only result.
    if inv_iters > 0:
        iteration = inv_iters
        checkpoint_name = f"{image_name}_iter{iteration}"
        runner.save(iteration=iteration, is_latest=False)
        ...
        # Infer GS from the batch (take first element if it's a list from dataloader)
