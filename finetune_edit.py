...
# Usage example:
# python finetune_ohta.py infer-handavatar-hand \
#   --checkpoint-file=./checkpoint/iteration_12000.ckpt \
#   --input-dir=./example_data/hand_image.jpg \
#   --handavatar-path=/path/to/InterHand/dataset \
#   --animate-to-handavatar
...
flags.DEFINE_integer('iter', '800', 'Finetuning iteration')
flags.DEFINE_string('edit', 'True', 'Finetuning iteration')
flags.DEFINE_string('handavatar-path', None, 'HandAvatar dataset path for animating GS to test poses')
...
def main(argv):
    ...
    # Extract image name for checkpoint naming
    if multi_image_mode or not img_glob:
        image_name = "default"
    else:
        # Single image mode - extract name without extension
        image_name = os.path.splitext(os.path.basename(img_glob[0]))[0]
    
    # Determine base output root: prefer explicit --output-path like test_wild, fallback to FLAGS.output_dir
    ...
    print(f"\033[94mFine-tuning Configuration:\033[0m")
    ...
    import re
    # Ensure test_iters is always an int
    ...
    try:
        if hasattr(runner, 'load_checkpoint'):
            # Prefer fine-tune aware loading when available
        ...
        # Fallback to base load and print error for visibility
        print(f"\033[91m[Load Debug] TypeError during finetune_model/load: {e}\033[0m")
        runner.load(iteration=test_iters, is_latest=False, checkpoint_path=cli_checkpoint)
    
    print(f"\033[91m[========== Running Finetune ! Loading model for iteration {test_iters} ==========]\033[0m")

    # *** CRITICAL: Reset checkpoint_path AFTER loading pretrained model ***
    # runner.load() may have overwritten checkpoint_path, so we set it again to our fine-tuning directory
    runner.checkpoint_path = image_checkpoint_dir
    ...
    if FLAGS.only_eval:
        ...
        # Evaluate over handavatar_dataloader if available, otherwise skip
        if handavatar_dataloader is not None:
            for idx, test_batch in enumerate(handavatar_dataloader):
                with tqdm(
                    total=1,
                    desc=f"Eval {eval_iter} | Sample {idx+1}/{len(handavatar_dataloader)}",
                    leave=True,
                    position=idx + 1,
                    dynamic_ncols=True,
                ) as pbar:
            ...
            print(f"\033[91m[Warning] handavatar_dataloader is None, skipping evaluation\033[0m")
    ...
    for iteration in pbar:
        ...
        if FLAGS.edit == 'True':
            runner.run_edit(batch=batches, scaler=scaler, iteration=iteration, writer=writer, pbar=pbar, total_iters=FLAGS.iter)    # hand
        else:
            runner.run(batch=batches, scaler=scaler, iteration=iteration, writer=writer, pbar=pbar)    # hand

        # Save checkpoint every 100 iterations after iteration 400
        # if iteration % 500 == 0:
        # if iteration % 1000 == 0  or iteration == FLAGS.iter:
        if iteration == FLAGS.iter:
        # if iteration >= 400 and iteration % 100 == 0:
            # Save checkpoint with image name (directory already set via hand_model.checkpoint_path)
            ...
            # Reload the saved checkpoint for testing
            runner.load(iteration=iteration, is_latest=False, checkpoint_path=image_checkpoint_dir)
            
            print(f"\033[91m[========== Running test ! Testing {image_name} at iteration {iteration} ==========]\033[0m")
            ...
            # Save test metrics for this iteration
            current_metrics = {
                'image_name': image_name,
            ...
            # Save metrics directly in checkpoint directory
