...
import trimesh
from tools_utils.model.smplx.mis_utils import edge_subdivide
import torch
import numpy as np
import json
...
_SUBDIV_CACHE_DIR = os.environ.get("MANO_SUBDIV_CACHE_DIR", _DEFAULT_SUBDIV_CACHE_DIR)
os.makedirs(_SUBDIV_CACHE_DIR, exist_ok=True)
...
def sub_mano(mano, t, pretrain=None):
    ...
    try:
        ...
        if os.path.exists(cache_path):
            ...
            print(f"load cached subdivided mano from {cache_path}")
            return data['mano'], data['edges'], data['spirals']
    except Exception:
        pass
    ...
    mano.update_seal()
    # attempt to save to cache for future runs
    try:
        cache_path = os.path.join(_SUBDIV_CACHE_DIR, f"mano_subdiv_{t}.pth")
...
# def sub_mano_in_smplx(v_template, faces, mano, t, pretrain=None):
    for i in range(t):
        # n_verts = mano.v_template.shape[0]
        if i == 0:
            n_verts = v_template.shape[0]
            verts, faces, edges = edge_subdivide(vertices=v_template, faces=mano.faces)
        else:
            n_verts = mano.v_template.shape[0]
            verts, faces, edges = edge_subdivide(vertices=mano.v_template, faces=mano.faces)
        if i==t-1:
