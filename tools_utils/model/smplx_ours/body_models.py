...
from .lbs import (
    lbs, vertices2landmarks, find_dynamic_lmk_idx_and_bcoords, blend_shapes,inverse_pts,forward_pts)
...
class MANO(SMPL):
    ...
    def update_seal(self):
        if not self.seal:
            return
        if self.v_template.shape[0]==12337:
            if self.is_rhand:
                self.circle_v_id = torch.tensor([117, 3750, 971, 3747, 119, 3757, 974, 3762, 120, 4982, 1315, 4987, 108, 3667, 947, 3664, 79, 3461, 889, 3458, 78, 12252, 3074, 12249, 121, 12264, 3077, 12261, 214, 12276, 3080, 12273, 215, 12288, 3083, 12285, 279, 12298, 3085, 12295, 239, 4650, 1226, 4656, 234, 4608, 1214, 4614, 92, 3545, 913, 3542, 38, 5579, 1452, 5576, 122, 5475, 1431, 5472, 118, 3737, 967, 3734]).long()
            else:
                self.circle_v_id = torch.tensor([12267, 215, 12277, 3081, 12279, 279, 12298, 3085, 12295, 239, 4661, 1227, 4656, 234, 4606, 1212, 4609, 92, 3533, 911, 3536, 38, 5576, 1452, 5579, 122, 5476, 1432, 5471, 118, 3738, 968, 3733, 117, 3750, 971, 3747, 119, 3758, 973, 3761, 120, 4992, 1316, 4989, 108, 3657, 946, 3660, 79, 3449, 887, 3452, 78, 12241, 3072, 12243, 121, 12253, 3075, 12255, 214, 12265, 3078]).long()
        self.faces_tensor_seal = self.seal_mesh(faces=self.faces_tensor, left=not self.is_rhand)
        self.faces_seal = self.faces_tensor_seal.numpy()
    ...
    def seal_mesh(self, verts=None, faces=None, left=False):
        ...
        if verts is not None:
            center = (verts[:, self.circle_v_id, :]).mean(dim=1, keepdim=True)
            verts = torch.cat([verts, center], dim=1)
            ret.append(verts)
    ...
    def forward_pts(self, pnts_c, canonical_centers,new_centers, betas, transformations, pose_feature, shapedirs, posedirs, lbs_weights, dtype=torch.float32, mask=None):
        ...
        if self.scale is not None:
            pnts_c = pnts_c / self.scale
        if self.center_id is not None:
            pnts_c = pnts_c + canonical_centers
        pnts_c_original = inverse_pts(pnts_c, canonical_exp.expand(num_points, -1), self.canonical_transformations.expand(num_points, -1, -1, -1).cuda(), self.canonical_pose_feature.expand(num_points, -1).cuda(), shapedirs, posedirs, lbs_weights, dtype=dtype)
        pnts_p = forward_pts(pnts_c_original, betas, transformations, pose_feature, shapedirs, posedirs, lbs_weights, dtype=dtype)
        if self.center_id is not None:
            pnts_p = pnts_p - new_centers
        if self.scale is not None:
