...
from model.handavatar.core.nets.handavatar.component_factory import \
...
class Network(nn.Module):
    def __init__(self):
        ...
        # manohd
        self.mano = smplx.create(**cfg.smpl_cfg).requires_grad_(False)
        smpl = smplx.create(**cfg.smpl_cfg).requires_grad_(False)
        if not cfg.smpl_cfg.is_rhand:
            self.mano.shapedirs[:,0,:] *= -1
        if cfg.smpl_cfg['manohd']>0:
            print('MANO-HD in Model')
            smpl_body ,_ ,_ = sub_mano(smpl, cfg.smpl_cfg['manohd'])
    ...
    def forward(self,
        ...
        impl_code = self.smpl_body.coap.encode_body(smpl_output)
        impl_code['v_template']=self.smpl_body.v_template

        posed_res = self.mano(
            dst_shape.float(),
            dst_global_orient.float(),
            dst_posevec.float(),
            return_verts=True)
        # joints = posed_res.joints[0]
        impl_code['global_points'] = torch.cat([impl_code['global_points'], posed_res.vertices], dim=1)
        # impl_code['global_points'] = smpl_output.vertices[0]
