...
def example_meta_data(args):
    ...
    dataset_path = args.hanco_path
    with open(os.path.join(dataset_path, "meta.json"), "r") as f:
        meta = json.load(f)

    subject_id_meta = meta["subject_id"]   # [1518][num_frames]

    frames_by_subject = defaultdict(list)

    num_seq = len(subject_id_meta)

    for sid in range(num_seq):
        num_frames = len(subject_id_meta[sid])
        for fid in range(num_frames):
            subj = subject_id_meta[sid][fid]
            if subj is None:
                continue
            frames_by_subject[int(subj)].append((sid, fid))

    subject_ids = sorted(frames_by_subject.keys())
    print(subject_ids[:5])
...
def example_show_data(args, sid):
    ...
    for fid in range(len(meta_data['is_train'])):
        ...
        for j, img in enumerate(rgb_list):
            ...
            ax[j].set_yticks([], [])
        plt.show()
        # save_path = os.path.join('./', f'seq{sid:04d}_fid{fid:08d}.png')
        # plt.tight_layout()
...
def example_show_keypoints(args, sid, fid, cid):
    ...
    ax.imshow(img)
    plt.show()
    save_path = os.path.join('./', f'keypoints-seq{sid:04d}_fid{fid:08d}.png')
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
...
def example_show_shape(args, sid, fid, cid):
    ...
    plt.show()
    save_path = os.path.join('./', f'mano-seq{sid:04d}_fid{fid:08d}.png')
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
...
if __name__ == '__main__':
    ...
    example_show_shape(args, sid=110, fid=0, cid=0)
